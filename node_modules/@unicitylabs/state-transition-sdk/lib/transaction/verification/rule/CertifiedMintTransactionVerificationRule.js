...
export class CertifiedMintTransactionVerificationRule {
     ...
     */
    static async verify(trustBase, predicateVerifier, mintJustificationVerifier, genesis) {
        const results = [];
        if (!genesis.networkId.equals(trustBase.networkId)) {
            ...
            return new VerificationResult('CertifiedMintTransactionVerificationRule', VerificationStatus.FAIL, 'Mint network does not match trust base.', results);
        }
        ...
        let result = EncodedPredicate.equals(expectedLockScript, genesis.inclusionProof.certificationData?.lockScript)
        ...
        if (result.status !== VerificationStatus.OK) {
            return new VerificationResult('CertifiedMintTransactionVerificationRule', VerificationStatus.FAIL, 'Invalid lock script', results);
        }
        ...
        if (result.status !== InclusionProofVerificationStatus.OK) {
            return new VerificationResult('CertifiedMintTransactionVerificationRule', VerificationStatus.FAIL, `Inclusion proof verification failed: ${result.status?.toString()}`, results);
        }
        result = await mintJustificationVerifier.verify(genesis);
        results.push(result);
        if (result.status !== VerificationStatus.OK) {
            return new VerificationResult('CertifiedMintTransactionVerificationRule', VerificationStatus.FAIL, 'Invalid mint justification', results);
        }
        return new VerificationResult('CertifiedMintTransactionVerificationRule', VerificationStatus.OK, '', results);
    }
}
//# sourceMappingURL=CertifiedMintTransactionVerificationRule.js.map