...
export class CertifiedUnicityIdMintTransactionVerificationRule {
     ...
     */
    static async verify(trustBase, predicateVerifier, genesis, issuerPublicKey = null) {
        const results = [];
        if (issuerPublicKey !== null) {
            ...
            if (!EncodedPredicate.equals(expectedLockScript, genesis.lockScript)) {
                ...
                return new VerificationResult('CertifiedUnicityIdMintTransactionVerificationRule', VerificationStatus.FAIL, 'Lock script does not match expected unicity-id issuer.', results);
            }
            results.push(new VerificationResult('IsLockScriptValidVerificationRule', VerificationStatus.OK));
        }
        const result = await InclusionProofVerificationRule.verify(trustBase, predicateVerifier, genesis.inclusionProof, genesis);
        results.push(result);
        if (result.status !== InclusionProofVerificationStatus.OK) {
            return new VerificationResult('CertifiedUnicityIdMintTransactionVerificationRule', VerificationStatus.FAIL, `Inclusion proof verification failed: ${result.status?.toString()}`, results);
        }
        return new VerificationResult('CertifiedUnicityIdMintTransactionVerificationRule', VerificationStatus.OK, '', results);
    }
}
//# sourceMappingURL=CertifiedUnicityIdMintTransactionVerificationRule.js.map