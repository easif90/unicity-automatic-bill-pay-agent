...
export class SplitMintJustification {
    ...
    static async fromCBOR(bytes) {
        ...
        return SplitMintJustification.create(await Token.fromCBOR(data[0]), CborDeserializer.decodeArray(data[1]).map((proof) => SplitAssetProof.fromCBOR(proof)));
    }
    /**
     ...
     */
    toCBOR() {
        return CborSerializer.encodeTag(SplitMintJustification.CBOR_TAG, CborSerializer.encodeArray(this.token.toCBOR(), CborSerializer.encodeArray(...this._proofs.map((proof) => proof.toCBOR()))));
    }
}
//# sourceMappingURL=SplitMintJustification.js.map