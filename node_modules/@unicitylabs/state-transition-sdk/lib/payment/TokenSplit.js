...
export class TokenSplit {
     ...
     */
    static async split(token, decodePaymentData, requests, burnStateMask) {
        ...
        const requestsWithTokenId = new Map();
        ...
        const burnTransaction = await TransferTransaction.create(token, burnPredicate, burnStateMask ?? crypto.getRandomValues(new Uint8Array(32)));
        const tokens = Array.from(requestsWithTokenId.values()).map(([request, tokenId]) => new SplitToken(networkId, request.recipient, request.tokenType, request.salt, request.assets, request.assets
            .toArray()
            .map((asset) => SplitAssetProof.create(asset.id, aggregationRoot.getPath(asset.id.toBitString().toBigInt()), assetTreeRoots.get(HexConverter.encode(asset.id.bytes)).getPath(tokenId.toBitString().toBigInt())))));
        return {
            burn: {
...
//# sourceMappingURL=TokenSplit.js.map