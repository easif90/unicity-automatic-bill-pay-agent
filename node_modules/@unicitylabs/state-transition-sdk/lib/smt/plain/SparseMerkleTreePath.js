...
export class SparseMerkleTreePath {
    ...
    static fromCBOR(bytes) {
        ...
        return new SparseMerkleTreePath(DataHash.fromImprint(CborDeserializer.decodeByteString(data[0])), steps.map((step) => SparseMerkleTreePathStep.fromCBOR(step)));
    }
    /**
    ...
    async verify(stateId) {
        ...
        if (step.path > 1n) {
            const hash = await new DataHasher(this.root.algorithm)
                .update(CborSerializer.encodeArray(CborSerializer.encodeByteString(BigintConverter.encode(step.path)), CborSerializer.encodeNullable(step.data, CborSerializer.encodeByteString)))
                .digest();
        ...
        for (let i = 1; i < this.steps.length; i++) {
            ...
            const hash = await new DataHasher(this.root.algorithm)
                .update(CborSerializer.encodeArray(CborSerializer.encodeByteString(BigintConverter.encode(step.path)), CborSerializer.encodeNullable(left, CborSerializer.encodeByteString), CborSerializer.encodeNullable(right, CborSerializer.encodeByteString)))
                .digest();
            currentData = hash.data;
...
//# sourceMappingURL=SparseMerkleTreePath.js.map