...
export class SparseMerkleSumTree {
    factory;
    left = Promise.resolve(null);
    right = Promise.resolve(null);
    /**
    ...
    async addLeaf(path, data, value) {
        ...
        const newBranchPromise = branchPromise.then((branch) => branch ? this.buildTree(branch, path, data, value) : new PendingLeafBranch(path, data, value));
        if (isRight) {
     ...
     */
    async calculateRoot() {
        this.left = this.left.then((branch) => (branch ? branch.finalize(this.factory) : Promise.resolve(null)));
        this.right = this.right?.then((branch) => (branch ? branch.finalize(this.factory) : Promise.resolve(null)));
    ...
    buildTree(branch, remainingPath, data, value) {
        ...
        if (isRight) {
            return new PendingNodeBranch(branch.path, branch.left, this.buildTree(branch.right, remainingPath >> commonPath.length, data, value));
        }
        return new PendingNodeBranch(branch.path, this.buildTree(branch.left, remainingPath >> commonPath.length, data, value), branch.right);
    }
}
//# sourceMappingURL=SparseMerkleSumTree.js.map