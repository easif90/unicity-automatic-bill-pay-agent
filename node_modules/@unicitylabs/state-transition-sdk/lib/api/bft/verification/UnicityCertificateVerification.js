 ...
 */
class UnicityCertificateVerificationResult extends VerificationResult {
...
export class UnicityCertificateVerification {
     ...
     */
    static async verify(trustBase, inclusionProof) {
        const results = [];
        ...
        result = await UnicitySealQuorumSignaturesVerificationRule.verify(trustBase, inclusionProof.unicityCertificate.unicitySeal);
        results.push(result);
        if (result.status !== VerificationStatus.OK) {
            return UnicityCertificateVerificationResult.fail(results);
        }
...
//# sourceMappingURL=UnicityCertificateVerification.js.map